...
Bedrock Demo — North Zone (Coast Mountains, BC): a fabricated block model
rendered on real Esri terrain.

- **168,013 mineralized blocks** — 6,336,972 t @ 2.587 g/t AuEq = **526,981 oz**
- **22 vein domains**, each isolatable with its own grade-tonnage
...
or vein domain · cut-off ladder · isolation of any of the 22 vein domains with
...
python3 tools/extract_surfaces.py 8                       # → data/demo_surfaces.json
python3 tools/build_present.py                            # → pit/index.html + pit/sw.js
python3 tools/capture_thumbs.py                           # → tools/assets/thumbs.json
python3 tools/build_present.py                            # again, to embed them
```

The generated source CSV (25 MB) is git-ignored — tools/make_demo_model.py reproduces it exactly from its seed. `extract_blocks.py` reads it
...
miss. The `vein` column in `demo_blocks_v2.csv` is the *dominant* domain and is a
...
- **Resource class labels are fabricated.** So is everything they classify. The
  generator assigns 1/2/3 by how well tested a block would notionally be — a
  drilled core, a step out, then depth — and there is no drilling behind any of
  it, so there is no technical report to check them against. The split is
  Measured 33,436, Indicated 84,681, Inferred 3,540. The viewer surfaces this as a caveat and it
  also appears on every exported slide.
- **Drill holes AND all site features are FABRICATED.** `data/synthetic/` is generated by
  `tools/make_synthetic_drills.py`; no drilling happened and these are not real
  results. They exist so the drill-trace feature could be built before real
...
- **The second deposit is FABRICATED.** There is no South Zone.
...
block and chapters all come from the payload. Verified by hydrating the demo artifact through `dashboard/lib/extract.js` and landing on the baked deck's
